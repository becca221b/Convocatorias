...
{
    public sealed class PeriodoGetVigenteUseCase
    {
        private readonly IPeriodoRepository _repository;
        public PeriodoGetVigenteUseCase(IPeriodoRepository repository)
        {
            _repository = repository;
        }

        public async Task<PeriodoResponse?> ExecuteAsync(CancellationToken ct = default)
        {
            var periodo = await _repository.GetVigenteAsync(DateTime.UtcNow, ct);
